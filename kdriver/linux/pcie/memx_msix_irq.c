...
static DECLARE_COMPLETION(thread_ready);
...
static int memx_polling_message_handler(void *data) {
	struct memx_pcie_dev *memx_dev = data;
	s32 chip_id = 0;
	u32* data_from_buffer = 0;

	dma_sync_single_for_cpu(&memx_dev->pDev->dev, (dma_addr_t)(memx_dev->mpu_data.hw_info.fw.rx_dma_coherent_buffer_base + MEMX_POLLING_ADDR_START), POLLING_ADDR_BYTE_OFFSET, DMA_BIDIRECTIONAL);
	data_from_buffer = (u32*)MEMX_GET_POLLING_ADDR(memx_dev, HOST_CMD_DONE_ACK_ID);
	if (*data_from_buffer > 0) {
		// pr_info("fw ack *data_from_buffer %d\r\n", *data_from_buffer);
		spin_lock(&memx_dev->mpu_data.fw_ctrl.lock);
		...
		*data_from_buffer = 0;
		dma_sync_single_for_device(&memx_dev->pDev->dev, (dma_addr_t)(memx_dev->mpu_data.hw_info.fw.rx_dma_coherent_buffer_base + MEMX_POLLING_ADDR_START), POLLING_ADDR_BYTE_OFFSET, DMA_BIDIRECTIONAL);
	}

	for (chip_id = 0; chip_id < MAX_CHIP_NUM; chip_id++) {
		dma_sync_single_for_cpu(&memx_dev->pDev->dev, (dma_addr_t)(MEMX_GET_POLLING_ADDR(memx_dev, EGR_MSIX_VECTOR_OFFS(chip_id))), POLLING_ADDR_BYTE_OFFSET, DMA_BIDIRECTIONAL);
		data_from_buffer = (u32*)MEMX_GET_POLLING_ADDR(memx_dev, EGR_MSIX_VECTOR_OFFS(chip_id));
		if (*data_from_buffer > 0) {
			if (!kfifo_in_locked(&memx_dev->rx_msix_fifo, &chip_id, sizeof(s32), &memx_dev->mpu_data.rx_ctrl.lock))
					pr_err("memryx: isr: kfifo_in fail, rx_msix_fifo is full\n");

			wake_up_interruptible(&memx_dev->mpu_data.rx_ctrl.wq);

			*data_from_buffer = 0;
			dma_sync_single_for_device(&memx_dev->pDev->dev, (dma_addr_t)(MEMX_GET_POLLING_ADDR(memx_dev, EGR_MSIX_VECTOR_OFFS(chip_id))), POLLING_ADDR_BYTE_OFFSET, DMA_BIDIRECTIONAL);
		}

		dma_sync_single_for_cpu(&memx_dev->pDev->dev, (dma_addr_t)(MEMX_GET_POLLING_ADDR(memx_dev, IGR_MSIX_VECTOR_OFFS(chip_id))), POLLING_ADDR_BYTE_OFFSET, DMA_BIDIRECTIONAL);
		data_from_buffer = (u32*)MEMX_GET_POLLING_ADDR(memx_dev, IGR_MSIX_VECTOR_OFFS(chip_id));
		if (*data_from_buffer > 0) {
			spin_lock(&memx_dev->mpu_data.tx_ctrl[chip_id].lock);
			memx_dev->mpu_data.tx_ctrl[chip_id].indicator = chip_id;
			spin_unlock(&memx_dev->mpu_data.tx_ctrl[chip_id].lock);
			...
			*data_from_buffer = 0;
			dma_sync_single_for_device(&memx_dev->pDev->dev, (dma_addr_t)(MEMX_GET_POLLING_ADDR(memx_dev, IGR_MSIX_VECTOR_OFFS(chip_id))), POLLING_ADDR_BYTE_OFFSET, DMA_BIDIRECTIONAL);}
	}

	return 0;
}

static int polling_thread_worker(void *data)
{
	struct memx_pcie_dev *memx_dev = data;
	complete(&memx_dev->mpu_data.thread_ready); // Optional for waiting thread ready flag

	wait_event_interruptible(
		memx_dev->mpu_data.polling_ctrl.wq,
		READ_ONCE(memx_dev->mpu_data.polling_ctrl.is_abort) ||
		READ_ONCE(memx_dev->mpu_data.polling_kick) ||
		kthread_should_stop()
	);

	if (kthread_should_stop() || READ_ONCE(memx_dev->mpu_data.polling_ctrl.is_abort)) return 0;


	while (!kthread_should_stop() && !READ_ONCE(memx_dev->mpu_data.polling_ctrl.is_abort)) {
		memx_polling_message_handler(memx_dev);
...
s32 memx_init_polling_thread(struct memx_pcie_dev *memx_dev)
{
	init_waitqueue_head(&memx_dev->mpu_data.polling_ctrl.wq);
	spin_lock_init(&memx_dev->mpu_data.polling_ctrl.lock);
	init_completion(&memx_dev->mpu_data.thread_ready);

	memx_dev->mpu_data.polling_ctrl.is_abort = false;
	memx_dev->mpu_data.polling_kick = false;
	memx_dev->mpu_data.polling_thread = kthread_run(polling_thread_worker, memx_dev, "polling_thread_worker");

    if (IS_ERR(memx_dev->mpu_data.polling_thread))
        return PTR_ERR(memx_dev->mpu_data.polling_thread);

    wait_for_completion(&memx_dev->mpu_data.thread_ready); // Optional for waiting thread ready flag

	WRITE_ONCE(memx_dev->mpu_data.polling_kick, true);
	wake_up_interruptible(&memx_dev->mpu_data.polling_ctrl.wq);

	return 0;
}

void memx_remove_polling_thread(struct memx_pcie_dev *memx_dev)
{
    WRITE_ONCE(memx_dev->mpu_data.polling_ctrl.is_abort, true);

    wake_up_all(&memx_dev->mpu_data.polling_ctrl.wq);

    if (memx_dev->mpu_data.polling_thread) {
        kthread_stop(memx_dev->mpu_data.polling_thread);
        memx_dev->mpu_data.polling_thread = NULL;
    }
}
